        ...
        if optimizer_idx == 1:
            ...
            with autocast(enabled=False):  # use float32 for the criterion
                loss_dict = criterion[optimizer_idx](
                    ...
                    seg_wav=self.model_outputs_cache["waveform_seg"],
                    raw_text=self.model_outputs_cache["raw_text"],

                    # MultiScaleSTFTLoss
                    # y_wav=self.model_outputs_cache["model_outputs"],
                    # y_hat_wav=self.model_outputs_cache["waveform_seg"]
                )
